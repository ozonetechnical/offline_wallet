(function() {
    ...
    DOM.accxprv=$('.accountxprv_');
    DOM.accxpub=$('.accountxpub_');
    ...
    DOM.changenetwork = $('.changenetwork li');
    DOM.buttonnext = $('#getaddress');

    function init() {
        // Events
        DOM.buttonnext.on('click',getnextaddress);
        DOM.changenetwork.on('click',changemynetwork);
    ...
    function getnextaddress(){
      ...
      var indexrow = lastIndexInTable() + 1;
      displayAddresses(indexrow,1);
    }
    ...
    function changemynetwork(e){
      var mynet = e.target.getAttribute('data-network');
      $('.network').val(mynet).trigger("change");
    }
    ...
    function generateRandomPhrase() {
        ...
        var numWords = 24; //parseInt(DOM.generatedStrength.val());
    ...
    function displayBip44Info() {
        ...
        DOM.accxprv.val(accountXprv);
        DOM.accxpub.val(accountXpub);

        displayAddresses(0,1);
    ...
    function showRow(){
      var rowindex = parseInt(DOM.rowsToShow.val());
      displayAddresses(rowindex,1);
    }
    ...
    function addAddressToList(indexText, address, pubkey, privkey) {
        ...
        var thisrow = row.find("svg");
        thisrow.attr('data-prv',privkey);
        thisrow.attr('data-addr',address);

        thisrow.click(function(e){
          var addr = e.target.getAttribute('data-addr');
          var prvkey = e.target.getAttribute('data-prv');
          // console.log(prvkey);
           if (addr){
             $('.rc').text(addr);
             $('.rc').attr('data-prv',prvkey);
             $('#rc').html('');
             var qrcode = new QRCode("rc");
             qrcode.makeCode("bitcoin:" + addr);

             var qrsync = new QRCode("qr-sync");
             qrsync.makeCode("https://offline.o.zone/sync/" + addr);

              $('.page').slideUp('fast');
              $('.action').slideDown();
               }
        });
        // var sendbtn = row.find(".btn .sendbtn");
        // sendbtn.attr('data-prv',privkey);
        // sendbtn.attr('data-addr',address);
        // sendbtn.on('click',function(e){
        //   var tx = coinjs.transaction();
        //   var addr = e.target.getAttribute('data-addr');
        //   var prv = e.target.getAttribute('data-prv');
        //   tx.addinput(txhash, n, scr, seq);
        //   tx.addoutput(addr, '125');
        //   var tx2 = coinjs.transaction();
  			// 	var txunspent = tx2.deserialize(tx.serialize());
        //   var signed = txunspent.sign(prv);
        //   console.log(signed);
        // });
        // Content
        indexnumber = indexText.substr(indexText.lastIndexOf("/") + 1);
        indexCell.text(indexnumber);
    ...
    //     var myNetworks = [19,63,64,43,97,116,166,182,184,191];
    //     for (var i=0; i<networks.length; i++) {
    //        var n = myNetworks.includes(i)
    //         if (n) {
    //
